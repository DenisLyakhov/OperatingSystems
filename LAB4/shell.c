...
int executecmds(struct cmdlist* __head) {
	...
	// If procres was established
        if (procres) {
            if (((curr->conjuction == CONJAND) && (WEXITSTATUS(status) != 0)) || ((curr->conjuction == CONJOR) && (WEXITSTATUS(status) == 0)))
                check = 0;
        }
